...
- 富文本编辑：Markdown 语法、LaTeX 公式 ( $...$ / $$...$$ )，从 Word 直接粘贴公式自动转换，编辑区字号独立调节
- 提词播放：自动滚动（WPM 速率可控）、手动键盘滚动（t²·⁵ 加速曲线）
- 镜像模式：第二屏幕输出，水平/垂直翻转，状态自动记忆
- 控制面板：浮动参数调节（字号 50-250px、行距、速度、边距、引导框透明度）
- 视线引导框：可拖拽定位，透明度可调（0-100%），F2 快捷键开关
- 提词中编辑：通过控制面板"编辑"按钮直接修改稿件，保持全屏/镜像状态，保存后自动恢复播放进度
- 快捷键全覆盖：空格播放、↑↓滚动、+/-调速、R重置、M镜像、F11全屏
- 焦点无感：鼠标在控制面板上时快捷键依然生效，滚轮也可正常滚动提词画面
...
### 打包版本
下载 [Releases](https://github.com/Dreamteaveler/Teleprompter/releases) 中的 `提词器1.09.zip`，解压出来后双击运行（无需安装 Python）
